...
from examples.stg.order_system_orders_dag.order_pg_saver import OrderPgSaver
...
class OrderLoader:
    ...
    WF_KEY = "example_ordersystem_order_origin_to_stg_workflow"
    ...
    def run_copy(self) -> int:
        ...
        with self.pg_dest.connection() as conn:
            ...
            self.log.info(f"Found {len(load_queue)} documents to sync from order collection.")
            if not load_queue:
            ...
            for d in load_queue:
                ...
                if i % self._LOG_THRESHOLD == 0:
                    self.log.info(f"processed {i} documents of {len(load_queue)} while syncing order.")
